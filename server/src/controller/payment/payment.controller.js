...
function getProviderConfig(provider) {
    ...
    throw new BadRequestError('Cong thanh toan khong hop le');
}
...
function ensureOrderAccess(payment, req) {
    ...
    if (!isAdmin(req) && ownerId !== userId) {
        throw new ForbiddenError('Ban khong co quyen truy cap thanh toan nay');
    }
}
...
async function updatePaymentState({ orderId, provider, status, transactionCode }) {
    ...
    if (!payment) {
        throw new NotFoundError('Khong tim thay giao dich thanh toan');
    }
    ...
    const nextOrderStatus =
        nextPaymentStatus === 'success' && payment.orders.order_status === 'pending'
            ? 'confirmed'
...
async function buildProviderPaymentUrl({ provider, order, payment, req }) {
    ...
    if (missingEnv.length > 0) {
        throw new BadRequestError(
            `Cong thanh toan ${getProviderDisplayName(provider)} chua duoc cau hinh day du. Thieu: ${missingEnv.join(
                ', '
            )}`
        );
    }
    ...
    if (provider === 'momo') {
        ...
        if (!response.data?.payUrl) {
            throw new BadRequestError(response.data?.message || 'Khong tao duoc lien ket thanh toan MoMo');
        }
    ...
    if (provider === 'zalopay') {
        ...
        const response = await axios.post(config.paymentUrl, null, {
            params: {
                ...
                description: `ZaloPay - Thanh toan don hang ${order.order_code}`,
                bank_code: '',
        ...
        if (!response.data?.order_url) {
            throw new BadRequestError(response.data?.return_message || 'Khong tao duoc lien ket thanh toan ZaloPay');
        }
...
class PaymentController {
    async gatewayPage(req, res) {
        return res.status(410).json({
            success: false,
            message: 'Da loai bo gateway sandbox noi bo. Hay goi /api/payments/:orderId/initiate de lay URL thanh toan that.',
        });
    }
    ...
    async createPayment(req, res) {
        ...
        if (!payment) {
            throw new NotFoundError('Khong tim thay ban ghi thanh toan');
        }

        ensureOrderAccess(payment, req);

        if (payment.orders.order_status === 'cancelled') {
            throw new BadRequestError('Khong the thanh toan don hang da huy');
        }
    ...
    async paymentCallback(req, res) {
        ...
        const redirectUrl =
            status === 'success'
                ...
                : `${getClientBaseUrl()}/checkout?orderId=${orderId}&payment=failed`;
    ...
    async updatePayment(req, res) {
        ...
        if (!validStatuses.includes(nextStatus)) {
            throw new BadRequestError('Trang thai thanh toan khong hop le');
        }
