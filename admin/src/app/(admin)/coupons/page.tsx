...
export default function CouponsPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        discountValue: '', minOrderValue: '', maxDiscountAmount: '', usageLimit: '', startsAt: '', expiresAt: '', status: 'active' as 'active' | 'inactive',
    });
    ...
    const paginatedCoupons = useMemo(() => coupons.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE), [coupons, currentPage]);

    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        event.preventDefault();
        setErrorMessage('');
        setSuccessMessage('');
        if (!formData.code.trim() || !formData.name.trim()) return setErrorMessage('Vui lòng nhập mã coupon và tên chương trình.');
        if (!formData.discountValue || Number(formData.discountValue) <= 0) return setErrorMessage('Giá trị giảm giá phải lớn hơn 0.');
        ...
        try {
            ...
            setFormData({ code: '', name: '', description: '', discountType: 'percentage', discountValue: '', minOrderValue: '', maxDiscountAmount: '', usageLimit: '', startsAt: '', expiresAt: '', status: 'active' });
        } catch (error) {
    ...
    const handleDelete = async (coupon: Coupon) => {
        ...
        try {
            ...
            if (expandedCouponId === coupon.id) setExpandedCouponId(null);
    ...
    const handlePageChange = (page: number) => {
        ...
        if (!nextPageItems.some((item) => item.id === expandedCouponId)) setExpandedCouponId(null);
    };
            ...
            <SectionCard title="Tạo mã giảm giá" description="Admin có thể tạo coupon để người dùng áp dụng tại bước thanh toán.">
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-2">
                        <input className="w-full rounded-2xl border border-[var(--border)] bg-white px-4 py-3 text-sm outline-none" placeholder="Mã coupon" value={formData.code} onChange={(event) => setFormData((prev) => ({ ...prev, code: event.target.value }))} />
                        <input className="w-full rounded-2xl border border-[var(--border)] bg-white px-4 py-3 text-sm outline-none" placeholder="Tên chương trình" value={formData.name} onChange={(event) => setFormData((prev) => ({ ...prev, name: event.target.value }))} />
                    </div>
                    <textarea className="w-full rounded-2xl border border-[var(--border)] bg-white px-4 py-3 text-sm outline-none" rows={3} placeholder="Mô tả" value={formData.description} onChange={(event) => setFormData((prev) => ({ ...prev, description: event.target.value }))} />
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <input type="number" min="0" value={formData.discountValue} onChange={(event) => setFormData((prev) => ({ ...prev, discountValue: event.target.value }))} className="w-full rounded-2xl border border-[var(--border)] bg-white px-4 py-3 text-sm outline-none" placeholder="Giá trị giảm" />
                    </div>
                    <div className="grid gap-4 md:grid-cols-2">
                        <input type="number" min="0" value={formData.minOrderValue} onChange={(event) => setFormData((prev) => ({ ...prev, minOrderValue: event.target.value }))} className="w-full rounded-2xl border border-[var(--border)] bg-white px-4 py-3 text-sm outline-none" placeholder="Đơn tối thiểu" />
                        <input type="number" min="0" value={formData.maxDiscountAmount} onChange={(event) => setFormData((prev) => ({ ...prev, maxDiscountAmount: event.target.value }))} className="w-full rounded-2xl border border-[var(--border)] bg-white px-4 py-3 text-sm outline-none" placeholder="Giảm tối đa" />
                    </div>
                    <div className="grid gap-4 md:grid-cols-2">
                        <input type="number" min="0" value={formData.usageLimit} onChange={(event) => setFormData((prev) => ({ ...prev, usageLimit: event.target.value }))} className="w-full rounded-2xl border border-[var(--border)] bg-white px-4 py-3 text-sm outline-none" placeholder="Số lượt tối đa" />
                    ...
                    <button type="submit" disabled={isSubmitting} className="w-full rounded-2xl bg-[var(--foreground)] px-4 py-3 text-sm font-semibold text-white disabled:opacity-70">{isSubmitting ? 'Đang tạo coupon...' : 'Tạo mã giảm giá'}</button>
                </form>
            </SectionCard>

            <SectionCard title="Danh sách coupon" description="Có phân trang và chỉ hiện detail khi bạn bấm vào coupon.">
                {isLoading ? (
                    <div className="rounded-2xl border border-[var(--border)] bg-[var(--panel-strong)] px-4 py-6 text-sm text-[var(--muted)]">Đang tải mã giảm giá...</div>
                ) : (
                                ...
                                <div key={coupon.id} className="rounded-2xl border border-[var(--border)] bg-white p-4">
                                    ...
                                    {isExpanded ? (
                                        <div className="mt-3 border-t border-[var(--border)] pt-3 text-sm text-[var(--muted)]">
                                            ...
                                            <p>Loại: {coupon.discount_type === 'percentage' ? `${Number(coupon.discount_value)}%` : formatCurrency(Number(coupon.discount_value))}</p>
                                            ...
                                            <p>Giảm tối đa: {coupon.max_discount_amount ? formatCurrency(Number(coupon.max_discount_amount)) : 'Không giới hạn'}</p>
                                            <p>Bắt đầu: {formatDateTime(coupon.starts_at)}</p>
                                            <p>Hết hạn: {formatDateTime(coupon.expires_at)}</p>
                                            <div className="mt-3 flex gap-2">
                                                <button type="button" onClick={() => handleToggleStatus(coupon)} className="rounded-2xl border border-[var(--border)] px-4 py-3 text-sm font-semibold text-[var(--foreground)]">{coupon.status === 'active' ? 'Tạm ngưng' : 'Kích hoạt'}</button>
                                                <button type="button" onClick={() => handleDelete(coupon)} className="rounded-2xl border border-[rgba(157,49,49,0.18)] px-4 py-3 text-sm font-semibold text-[var(--danger)]">Xóa</button>
                                            </div>
                                        </div>
                                    ) : null}
                                </div>
